...
export class VisualizationBuilder {
    ...
    public buildFurnitureVisualization(assetName: string, outputPath: string) {
        return new Promise<void>(async resolve => {
            ...
            let indexXml: any = fs.readFileSync(`${outputPath}/${assetName}/index.xml`);
            indexXml = xml2js(indexXml, {compact: true});
            let visualizationXml: any = fs.readFileSync(`${outputPath}/${assetName}/${assetName}_visualization.xml`);
            visualizationXml = xml2js(visualizationXml, {compact: true});
            let logicXml: any = fs.readFileSync(`${outputPath}/${assetName}/${assetName}_visualization.xml`);
